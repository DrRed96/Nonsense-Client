...
public class NetworkSystem {
    ...
    public static final LazyLoadBase<NioEventLoopGroup> NIO_EVENT_LOOP = new LazyLoadBase<>() {
        protected NioEventLoopGroup load() {
            return new NioEventLoopGroup(0, (new ThreadFactoryBuilder()).setNameFormat("Netty Server IO #%d").setDaemon(true).build());
        }
    };
    public static final LazyLoadBase<EpollEventLoopGroup> EPOLL_EVENT_LOOP = new LazyLoadBase<>() {
    ...
    public SocketAddress addLocalEndpoint() {
        ...
        synchronized (this.endpoints) {
            ...
            }).group(NIO_EVENT_LOOP.getValue()).localAddress(LocalAddress.ANY).bind().syncUninterruptibly();
                ...
                if (!networkmanager.hasNoChannel()) {
                    if (!networkmanager.isChannelOpen()) {
                        ...
                        try {
                            ...
                            networkmanager.sendPacket(new S40PacketDisconnect(chatcomponenttext), _ -> networkmanager.closeChannel(chatcomponenttext));
