...
public class GuiSpectator extends Gui implements ISpectatorMenuRecipient
{
    ...
    {
        long i = this.startTime - Minecraft.getSystemTime() + 5000L;
        return MathHelper.clamp_float((float)i / 2000.0F, 0.0F, 1.0F);
    }

    public void renderTooltip(ScaledResolution scaledRes, float p_175264_2_)
    {
        if (this.spectatorMenu != null)
        {
            ...
            else
            {
                ...
                this.zLevel = -90.0F;
                float f2 = (float)scaledRes.getScaledHeight() - 22.0F * f;
                SpectatorDetails spectatordetails = this.spectatorMenu.func_178646_f();
                this.draw(scaledRes, f, i, f2, spectatordetails);
                this.zLevel = f1;
            }
        }
    }

    protected void draw(ScaledResolution scaledRes, float opacity, int p_175258_3_, float p_175258_4_, SpectatorDetails details)
    {
        ...
        for (int i = 0; i < 9; ++i)
        {
            this.func_175266_a(i, scaledRes.getScaledWidth() / 2 - 90 + i * 20 + 2, p_175258_4_ + 3.0F, opacity, details.func_178680_a(i));
        }
