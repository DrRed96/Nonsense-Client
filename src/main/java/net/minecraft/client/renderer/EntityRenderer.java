...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private void orientCamera(float partialTicks) {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                if (!Nonsense.module(Tweaks.class).viewClip()) {
    ...
    public void setupCameraTransform(float partialTicks, int pass) {
        ...
        if (this.mc.gameSettings.viewBobbing && !Nonsense.module(Tweaks.class).minimalBobbing()) {
    ...
    public void renderHand(float partialTicks, int p_renderHand_2_, boolean p_renderHand_3_, boolean p_renderHand_4_, boolean p_renderHand_5_) {
        if (!this.debugView) {
            GlStateManager.matrixMode(5889);
            GlStateManager.loadIdentity();
            float f = 0.07F;
            ...
            if (p_renderHand_3_) {
                ...
                boolean flag1 = !ReflectorForge.renderFirstPersonHand(this.mc.renderGlobal, partialTicks, p_renderHand_2_);
    ...
    private void updateLightmap(float partialTicks) {
        if (this.lightmapUpdateNeeded) {
            ...
            if (worldclient != null) {
                ...
                Atmosphere atmosphere = Nonsense.module(Atmosphere.class);

                if (atmosphere.worldColor()) {
                    int color = atmosphere.worldColor.getRGB();
                    for (int i = 0; i < 256; ++i) {
                        this.lightmapColors[i] = color;
                    }
                } else {
                    for (int i = 0; i < 256; ++i) {
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.theWorld != null) {
                ...
                if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null) {
                    ...
                    if (this.mc.gameSettings.showDebugInfo) {
                        Lagometer.showLagometer(scaledRes);
                    }
                }
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    crashreportcategory.addCrashSectionCallable("Screen size", () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", scaledRes.getScaledWidth(), scaledRes.getScaledHeight(), EntityRenderer.this.mc.displayWidth, EntityRenderer.this.mc.displayHeight, scaledRes.getScaleFactor()));
