...
public class PlayerControllerMP {
    ...
    public boolean onPlayerRightClick(EntityPlayerSP player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos, EnumFacing side, Vec3 hitVec) {
        this.syncCurrentPlayItem();
        float hitX = (float) (hitVec.xCoord - (double) hitPos.getX());
        float hitY = (float) (hitVec.yCoord - (double) hitPos.getY());
        float hitZ = (float) (hitVec.zCoord - (double) hitPos.getZ());
        boolean flag = false;
        ...
        if (!this.mc.theWorld.getWorldBorder().contains(hitPos)) {
            ...
            if (this.currentGameType != WorldSettings.GameType.SPECTATOR) {
                ...
                if ((!player.isSneaking() || player.getHeldItem() == null) && iblockstate.getBlock().onBlockActivated(worldIn, hitPos, iblockstate, player, side, hitX, hitY, hitZ)) {
                    flag = true;
                }
            ...
            this.netClientHandler.addToSendQueue(new C08PacketPlayerBlockPlacement(hitPos, side.getIndex(), player.inventory.getCurrentItem(), hitX, hitY, hitZ));
            ...
            if (!flag && this.currentGameType != WorldSettings.GameType.SPECTATOR) {
                if (heldStack == null) {
                    ...
                    boolean flag1 = heldStack.onItemUse(player, worldIn, hitPos, side, hitX, hitY, hitZ);
                    heldStack.setItemDamage(i);
                    heldStack.stackSize = j;
                    return flag1;
                } else {
                    if (Nonsense.module(AntiDesync.class).placing()) {
                        return true;
                    }
                    return heldStack.onItemUse(player, worldIn, hitPos, side, hitX, hitY, hitZ);
                }
            } else {
                return true;
            }
    ...
    public boolean isPlayerRightClickingOnEntity(EntityPlayer playerIn, Entity targetEntity, MovingObjectPosition objectMouseOver) {
        this.syncCurrentPlayItem();
        Vec3 vec3 = new Vec3(objectMouseOver.hitVec.xCoord - targetEntity.posX, objectMouseOver.hitVec.yCoord - targetEntity.posY, objectMouseOver.hitVec.zCoord - targetEntity.posZ);
