...
public class KillAuraOld extends Module {
    ...
    public final Listener<EventClickAction> onClickAction = event -> {
        ...
        double range = this.isTargetValid ? attackRange : Math.max(attackRange, Math.max(rotRange.getDouble(), swingRange.getDouble()));

        MovingObjectPosition targetCheck = RotationUtil.rayCastEntityCheck(this.targetRotations, range, mc.thePlayer, this.target);

        if (targetCheck.entityHit == null) {
            this.targetRotations = RotationUtil.getRotations(this.getHitVec(this.target));
        }
        ...
        MovingObjectPosition rotateCheck = RotationUtil.rayCastEntityCheck(this.rotations, range, mc.thePlayer, this.target);

        if (rotateCheck.entityHit != null) {
            this.hitVec = rotateCheck.hitVec;
        } else {
...
//    };
    ...
    public final Listener<EventPreMotion> onPre = event -> {
        ...
        if (this.hitVec != null && this.hitVec.distanceTo(PlayerUtil.eyesPos()) <= this.rotRange.getDouble()) {
    ...
    private Vec3 getHitVec(EntityLivingBase entity) {
        return MathUtil.closestPoint(entity.getEntityBoundingBox(), PlayerUtil.eyesPos());
    }
    ...
    private void sortTargets(List<EntityLivingBase> targets) {
        targets.sort(switch (this.sorting.get()) {
            ...
            case ARMOR ->
                    Comparator.comparingInt(entity -> entity instanceof EntityPlayer ? ((EntityPlayer) entity).inventory.getTotalArmorValue() : (int) entity.getHealth());
